...
# storage with relative losses, we disallow outflows in the first time steps,
# so that the content is not transferred to storage_fixed
storage_relative = solph.components.GenericStorage(
    label="storage_relative",
    inputs={bus: solph.flows.Flow()},
    outputs={
        bus: solph.flows.Flow(nominal_value=4, max=[0, 0, 0, 0, 0, 0, 0, 1, 1])
    },
    nominal_storage_capacity=8,
    initial_storage_level=1,
...
results_df.rename(columns={"storage_content": "storage_fixed"}, inplace=True)
...
results_df["storage_relative"] = results[(storage_relative, None)]["sequences"]
