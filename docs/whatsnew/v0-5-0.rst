...
* Started API for distinct `Flow`, `InvestmentFlow`, `NonConvexFlow`, and `NonConvexInvestFlow`.
...
* Add `NonConvexInvestFlow` which provides the possibility to perform capacity optimization of assets considering their min/max loads, as well as their operation status.
