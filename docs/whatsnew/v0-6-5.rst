...
* ``_FakeSequence`` can no longer have a defined length.
  They have "every length" by convention. Thus, it can now be reliably cast;
  not to an array but to a scalar float. We expect no effect for end users.
* ``_FakeSequence`` have operators defined. We expect no effect for end users.
