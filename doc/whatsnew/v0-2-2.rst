...
* The storage API has been revised, even though it is still possible to use the
  ...
  (`Issue #491 <https://github.com/oemof/oemof/issues/491>`_).
...
* It is not possible to model one time step with oemof.solph. You have to
  model at least two timesteps (`Issue #306 <https://github.com/oemof/oemof/issues/306>`_). Please leave a comment if this bug affects you.
...
* The parameter_as_dict (former: param_results) do work with the views
  functions now (`Issue #494 <https://github.com/oemof/oemof/issues/494>`_).
