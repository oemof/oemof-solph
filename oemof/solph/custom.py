...
class ElectricalLineBlock(SimpleBlock):
    ...
    **The following variable are created:**
    ...
    def _create(self, group=None):
        ...
        self.ELECTRICAL_BUSES = po.po.Set(initialize=[n for n in m.es.nodes
                                       if isinstance(n, ElectricalBus)])

        def _voltage_angle_bounds(block, b, t):
            return b.v_min, b.v_max
        self.voltage_angle = po.po.Var(self.ELECTRICAL_BUSES, m.TIMESTEPS,
        ...
        self.electrical_flow = po.po.Constraint(group, noruleinit=True)

        self._equate_electrical_flows = po.po.Constraint(group, noruleinit=True)
...
class LinkBlock(SimpleBlock):
    ...
    **The following constraints are created:**
    ...
    def _create(self, group=None):
        ...
        self.relation = po.po.Constraint(group, noruleinit=True)
...
class GenericCAESBlock(SimpleBlock):
    ...
    def _create(self, group=None):
        ...
        self.GENERICCAES = po.Set(initialize=[n for n in group])

        # Compression: Binary variable for operation status
        self.cmp_st = po.Var(self.GENERICCAES, m.TIMESTEPS, within=Binary)

        # Compression: Realized capacity
        self.cmp_p = po.Var(self.GENERICCAES, m.TIMESTEPS,
                         within=NonNegativeReals)

        # Compression: Max. Capacity
        self.cmp_p_max = po.Var(self.GENERICCAES, m.TIMESTEPS,
                             within=NonNegativeReals)

        # Compression: Heat flow
        self.cmp_q_out_sum = po.Var(self.GENERICCAES, m.TIMESTEPS,
                                 within=NonNegativeReals)

        # Compression: Waste heat
        self.cmp_q_waste = po.Var(self.GENERICCAES, m.TIMESTEPS,
                               within=NonNegativeReals)

        # Expansion: Binary variable for operation status
        self.exp_st = po.Var(self.GENERICCAES, m.TIMESTEPS, within=Binary)

        # Expansion: Realized capacity
        self.exp_p = po.Var(self.GENERICCAES, m.TIMESTEPS,
                         within=NonNegativeReals)

        # Expansion: Max. Capacity
        self.exp_p_max = po.Var(self.GENERICCAES, m.TIMESTEPS,
                             within=NonNegativeReals)

        # Expansion: Heat flow of natural gas co-firing
        self.exp_q_in_sum = po.Var(self.GENERICCAES, m.TIMESTEPS,
                                within=NonNegativeReals)

        # Expansion: Heat flow of natural gas co-firing
        self.exp_q_fuel_in = po.Var(self.GENERICCAES, m.TIMESTEPS,
                                 within=NonNegativeReals)

        # Expansion: Heat flow of additional firing
        self.exp_q_add_in = po.Var(self.GENERICCAES, m.TIMESTEPS,
                                within=NonNegativeReals)

        # Cavern: Filling levelh
        self.cav_level = po.Var(self.GENERICCAES, m.TIMESTEPS,
                             within=NonNegativeReals)

        # Cavern: Energy inflow
        self.cav_e_in = po.Var(self.GENERICCAES, m.TIMESTEPS,
                            within=NonNegativeReals)

        # Cavern: Energy outflow
        self.cav_e_out = po.Var(self.GENERICCAES, m.TIMESTEPS,
                             within=NonNegativeReals)

        # TES: Filling levelh
        self.tes_level = po.Var(self.GENERICCAES, m.TIMESTEPS,
                             within=NonNegativeReals)

        # TES: Energy inflow
        self.tes_e_in = po.Var(self.GENERICCAES, m.TIMESTEPS,
                            within=NonNegativeReals)

        # TES: Energy outflow
        self.tes_e_out = po.Var(self.GENERICCAES, m.TIMESTEPS,
                             within=NonNegativeReals)

        # Spot market: Positive capacity
        self.exp_p_spot = po.Var(self.GENERICCAES, m.TIMESTEPS,
                              within=NonNegativeReals)

        # Spot market: Negative capacity
        self.cmp_p_spot = po.Var(self.GENERICCAES, m.TIMESTEPS,
                              within=NonNegativeReals)
        ...
        def cmp_p_constr_rule(block, n, t):
            ...
            return expr == 0
        self.cmp_p_constr = po.Constraint(
        ...
        self.cmp_p_max_constr = po.Constraint(
        ...
        self.cmp_p_max_area_constr = po.Constraint(
        ...
        self.cmp_st_p_min_constr = po.Constraint(
        ...
        self.cmp_st_p_max_constr = po.Constraint(
        ...
        self.cmp_q_out_constr = po.Constraint(
        ...
        def cmp_q_out_sum_constr_rule(block, n, t):
            return (self.cmp_q_out_sum[n, t] == self.cmp_q_waste[n, t] +
                    self.tes_e_in[n, t])
        self.cmp_q_out_sum_constr = po.Constraint(
        ...
        self.cmp_q_out_shr_constr = po.Constraint(
        ...
        def exp_p_constr_rule(block, n, t):
            ...
            return expr == 0
        self.exp_p_constr = po.Constraint(
        ...
        self.exp_p_max_constr = po.Constraint(
        ...
        self.exp_p_max_area_constr = po.Constraint(
        ...
        self.exp_st_p_min_constr = po.Constraint(
        ...
        self.exp_st_p_max_constr = po.Constraint(
        ...
        self.exp_q_in_constr = po.Constraint(
        ...
        def exp_q_fuel_constr_rule(block, n, t):
            ...
            return expr == 0
        self.exp_q_fuel_constr = po.Constraint(
        ...
        self.exp_q_in_sum_constr = po.Constraint(
        ...
        self.exp_q_in_shr_constr = po.Constraint(
        ...
        def cav_e_in_constr_rule(block, n, t):
            return (self.cav_e_in[n, t] ==
                    ...
                    n.params['cav_e_in_b'])
        self.cav_e_in_constr = po.Constraint(
        ...
        self.cav_e_out_constr = po.Constraint(
        ...
        def cav_eta_constr_rule(block, n, t):
            if t != 0:
                ...
                return (n.params['cav_eta_temp'] * self.cav_level[n, t] ==
                        m.timeincrement[t] *
                        (self.cav_e_in[n, t] - self.cav_e_out[n, t]))
        self.cav_eta_constr = po.Constraint(
        ...
        self.cav_ub_constr = po.Constraint(
        ...
        def tes_eta_constr_rule(block, n, t):
            if t != 0:
                ...
                return (self.tes_level[n, t] ==
                        m.timeincrement[t] *
                        (self.tes_e_in[n, t] - self.tes_e_out[n, t]))
        self.tes_eta_constr = po.Constraint(
        ...
        self.tes_ub_constr = po.Constraint(
