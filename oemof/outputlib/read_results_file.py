...
import logging
import numpy as np
from oemof.dblib import init_pahesmf as init
...
def read_results(var, main_dc):
    ...
    res_tables = rdb.fetch_columns(main_dc['basic'], 'information_schema',
    ...
    for key, table in zip(res_keys, res_tables):
        col_ls = bdb.fetch_column_names(main_dc['basic'],
        ...
        col_ls = bf.remove_from_list(col_ls, ['id', 'timestep', 'region'])
...
def read_info_table(var, main_dc):
    ...
    '''
    main_dc['res'].setdefault('info', {})
    var_type = init.check_parameter(var)
    main_dc['res']['info'] = dict(rdb.fetch_row(main_dc['basic'],
...
def restructure_results_generation_demand(main_dc):
    ...
    demand_dc['demand']['heat'] = ['twcb', 'dst0', 'thoi', 'thng', 'thhp']
