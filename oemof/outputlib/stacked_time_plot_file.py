    ...
    for i, c in zip(list(range(len(sp))), reversed(components)):
        ...
        sp[i] = ax_dc[ax].bar(
            np.arange(float(main_dc['res']['info']['plotstart']),
                      float(main_dc['res']['info']['plotend'] + 1)),
            tmp_res_dc[c],
            width=1,
            linewidth=0,
...
def lineplot_core(main_dc, reg, energy_type, ax_dc, ax):
    ...
    components = bf.cut_lists(main_dc['order']['demand'],
    ...
    for i, c in zip(list(range(len(lp))), reversed(components)):
        lp[i], = ax_dc[ax].plot(
            np.arange(
                float(main_dc['res']['info']['plotstart']),
...
def get_name_and_color(main_dc, u_type, energy_type):
    ...
    for comp in components:
        db_tmp = rdb.fetch_columns(
            main_dc['basic'], 'a_rli_corporate',
        ...
        main_dc['res']['color'].setdefault(comp, db_tmp['farbcode_hex'][0])
        main_dc['res']['name'].setdefault(comp, db_tmp[name_col][0])
...
def check_for_missing_components(main_dc, comp_type, energy_type):
    ...
    '''
    missing_comp = list(
        set(main_dc['res']['info'][energy_type][comp_type]).
...
def prepare_plot_dict(main_dc, region=None):
    ...
    main_dc['res'].setdefault('remove_empty_arrays', True)
    ...
    main_dc['res']['legend']['generation'].setdefault('title', 'Generation')
...
def multi_stacked_time_plot(main_dc, ax_dc, fig, element_list, regions):
    ...
    for etype in element_list:
        for region in regions:
            ...
            # create plots
            main_dc['res']['artist']['generation'] = stackplot_core(
                main_dc, region, etype, ax_dc, i)
            main_dc['res']['artist']['demand'] = lineplot_core(
                main_dc, region, etype, ax_dc, i)
