...
class Photovoltaic:
    ...
    def feedin_as_pd(self, **kwargs):
        ...
        return self.model.feedin_as_pd(**combined) * float(self.nominal_power)


class WindPowerPlant:
    def __init__(self, nominal_power, model=models.WindPowerPlant([]),
                 **attributes):
        """
        Unless you override the default model, the following attributes are
        required:
...
if __name__ == "__main__":
    ...
    class ConstantModell:
        ...
        def feedin(self, **ks):
            return [ks["nominal_power"]] * ks["steps"]
