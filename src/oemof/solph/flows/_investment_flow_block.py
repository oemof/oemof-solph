...
class InvestmentFlowBlock(ScalarBlock):
    ...
    * :attr:`om.InvestmentFlowBlock.investment_costs`, and
    ...
    * :attr:`om.InvestmentFlowBlock.period_investment_costs` (yielding a dict
      keyed by periods); note: this is not a Pyomo expression,
      but calculated, and
    * :meth:`om.InvestmentFlowBlock.costs`.
    ...
    def _create_constraints(self):
        ...
        def _total_capacity_rule(block):
            """Rule definition for determining total installed capacity"""
        ...
        self.total_capacity_rule = Constraint(
            self.INVESTFLOWS, m.CAPACITY_PERIODS, noruleinit=True
        )
        self.total_capacity_rule_build = BuildAction(rule=_total_capacity_rule)
