...
class GenericStorageBlock(ScalarBlock):
    ...
    **The following variables are created:**
    ...
    storage_content_timepoint
        ...
        be accessed by: `om.GenericStorageBlock.storage_content_timepoint[s, k, t]`
    ...
    * :attr:`fixed_costs` not 0
    ...
    def _create(self, group=None):
        ...
        if not m.TSAM_MODE:
            ...
            self.storage_content_timepoint = Var(
            ...
            for n in group:
                for p, k in m.TYPICAL_CLUSTERS:
                    self.storage_content_timepoint[n, p, k, 0] = 0
                    self.storage_content_timepoint[n, p, k, 0].fix()
                if n.initial_storage_level is not None:
                ...
                for p, i, g in m.TIMEINDEX_CLUSTER:
                    ...
                    rhs = (
                        ...
                        * (1 - n.loss_rate[t]) ** (g * m.timeincrement[tk])
                        + self.storage_content_timepoint[n, p, k, g]
                    )
        ...
        def _intra_storage_balance_rule(block, n, p, k, g):
            ...
            expr = 0
            expr += block.storage_content_timepoint[n, p, k, g + 1]
            expr += (
                -block.storage_content_timepoint[n, p, k, g]
        ...
        def _inter_storage_balance_rule(block, n, i):
            ...
            expr += -self.storage_content_timepoint[
        ...
        # Standard storage implementation for discrete time points
            ...
            def _inv_storage_init_content_max_rule(block, n):
                ...
                if not m.TSAM_MODE:
                    ...
                    lhs = block.storage_content_timepoint[n, 0, 0, 0]
