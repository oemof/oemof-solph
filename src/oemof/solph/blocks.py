    ...
    def _create(self, group=None):
        ...
        def _investvar_bound_rule(block, i, o):
            ...
            if (i, o) in self.CONVEX_MULTIPERIODFLOWS:
                for p in m.PERIODS:
                    return (m.flows[i, o, p].multiperiod.minimum,
                            m.flows[i, o, p].multiperiod.maximum)
            elif (i, o) in self.NON_CONVEX_MULTIPERIODFLOWS:
                for p in m.PERIODS:
                    return 0, m.flows[i, o, p].multiperiod.maximum
        ...
        def _min_invest_rule(block, i, o, p):
            """Rule definition for applying a minimum investment
            """
            expr = (m.flows[i, o, p].multiperiod.minimum *
        ...
        def _max_invest_rule(block, i, o, p):
            """Rule definition for applying a minimum investment
            """
    ...
    def _objective_expression(self):
        ...
        for i, o in self.CONVEX_MULTIPERIODFLOWS:
            for p in m.PERIODS:
                investment_costs += (
                    self.invest[i, o, p]
                    * m.flows[i, o, p].multiperiod.ep_costs)
        for i, o in self.NON_CONVEX_MULTIPERIODFLOWS:
            for p in m.PERIODS:
                investment_costs += (
                        ...
                        m.flows[i, o, p].multiperiod.ep_costs +
                        self.invest_status[i, o, p] *
                        m.flows[i, o, p].multiperiod.offset)
