...
if __name__ == "__main__":
  ...
  engine = create_engine("postgresql+psycopg2://USER:PASSOWRD@HOST/DATABASE")
  Session = sessionmaker(bind=engine)
  session = Session()

  # This example is for a spefic table from the "auswertung" database
  cpps = Table("geo_power_plant_bnetza_2014", MetaData(schema="vn"),
  ...
  old_coal_plants = session.query(components.Transformer
      ).filter(components.Transformer.year < "1950"
      ).filter(components.Transformer.type.like("%kohle%")
      ).all()
