...
# Plotly für das Dashboard
import plotly.graph_objects as go
from plotly.subplots import make_subplots

# HINWEIS: custom_constraints importieren wir hier NICHT, da wir kein FCR machen
...
try:
    ...
    data = pd.read_csv('market_data.csv', sep=';', decimal=',')

data['temp_time'] = pd.to_datetime(data['temp_time'])
data.set_index('temp_time', inplace=True)
data.index.freq = '15min'

# Test-Ausschnitt (z.B. 2 Tage = 192 Zeitschritte)
data_slice = data.head(192*2).copy()

# Preise bereinigen
data_slice['Spotmarkt_Preis'] = pd.to_numeric(data_slice['Spotmarkt_Preis'], errors='coerce').ffill().fillna(0)
# FCR Preis laden wir trotzdem für den Vergleich im Plot, auch wenn wir ihn nicht nutzen
data_slice['FCR_Leistungspreis'] = pd.to_numeric(data_slice['FCR_Leistungspreis'], errors='coerce').ffill().fillna(0)

print("Daten geladen (Modus: Nur Spotmarkt-Arbitrage).")


# =============================================================================
# 2. ENERGIESYSTEM DEFINIEREN
# =============================================================================
...
# Speicher (Standard Arbitrage Setup)
...
# =============================================================================
# 3. MODELL & CONSTRAINTS (HIER KEIN FCR!)
# =============================================================================
print("Erstelle Basis-Modell (Ohne FCR Constraints)...")
...
# B) FCR Ergebnisse simulieren (Alles Null, da nicht optimiert)
# Wir erstellen ein Array mit Nullen, damit die Plots identisch funktionieren
fcr_capacity_values = np.zeros(len(flow_in_values))

# C) Längen angleichen
min_len = min(len(data_slice), len(flow_in_values))

price_values = data_slice['Spotmarkt_Preis'].values[:min_len]
fcr_price_values = data_slice['FCR_Leistungspreis'].values[:min_len]
flow_in_values = flow_in_values[:min_len]
flow_out_values = flow_out_values[:min_len]
fcr_capacity_values = fcr_capacity_values[:min_len] # Bleibt Null
...
revenues_fcr = fcr_capacity_values * fcr_price_values * time_factor # Ist hier 0
...
print(f"--- ERGEBNIS (ARBITRAGE ONLY) ---")
print(f"Gesamtgewinn: {total_profit:.2f} EUR")
print(f"Davon aus FCR: {total_fcr_share:.2f} EUR (Sollte 0 sein)")
...
if PLOT_OPTIONS['1_Markt_FCR']:
    ...
    poly = ax1_b.fill_between(plot_index, 0, fcr_capacity_values, 
                              color='orange', alpha=0.3, hatch='///', label='FCR Reservierung')
