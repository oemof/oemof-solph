...
storesys.add(solph.Sink(label='exit', inputs={elbus: solph.Flow(variable_costs = sikosten)}))   

storesys.add(solph.Sink(label='eigenbedarf', inputs={elbus: solph.Flow(actual_value=vorgaben['aus'], fixed=True, nominal_value=500)}))


storesys.add(solph.Source(label='Backup', outputs={elbus: solph.Flow(variable_costs = sokosten)})) 

storesys.add(solph.Source(label='pv', outputs={elbus: solph.Flow(actual_value=vorgaben['ein'], fixed=True, nominal_value = 350)}))
...
speicher = solph.components.GenericStorage(
        ...
        #offset = 30,
        #slope = 25,
        inputs = {elbus: solph.Flow(investment= solph.Investment(), offset = 20, slope = 5, invmax = 2000000)},
        outputs = {elbus: solph.Flow(investment= solph.Investment(), offset = 20, slope = 10, invmax = 1000)},
