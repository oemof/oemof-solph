...
sys.add(solph.Sink(label='eigenbedarf', inputs={elbus: solph.Flow(actual_value=vorgaben['aus'], fixed=True, nominal_value=500)}))


sys.add(solph.Source(label='Backup', outputs={elbus: solph.Flow(variable_costs = 28)}))  



sys.add(solph.Source(label='gas', outputs={gabus: solph.Flow(variable_costs = 9)}))


 
sys.add(solph.Transformer(label='BHKW', inputs = {gabus: solph.Flow()},
                                        outputs = {elbus: solph.Flow( investment = solph.Investment(),
                                                                     invmax = 59000000,
                                                                     #invmax=60000000, 
